...
setEPS()
postscript(sprintf("nObs_%4d_msePlot.eps",nObs))
...
setEPS()
options(scipen=5)
postscript(sprintf("nObs_%04d_avergeComputationTime.eps",nObs))
...
setEPS()
options(scipen=5)
postscript(sprintf("nObs_%04d_timeStats_ratio.eps",nObs))

plot(timeStats[,"cr"],timeStats[,"m_zb"]/timeStats[,"m_wc"],type="l",lty=1, xlab="Censoring rate", ylab="Ratio of mean computation time")
grid()
